...
int Oattack(int &UHP, int &OPower) {
	UHP = (UHP - OPower);
	cout << "\nThe enemy attacks and deals " << OPower << "DAMAGE" << endl;
...
int Uattack(int& OHP, int& UPower) {
	...
	if (DriveTurnsLeft > 0) {
		OHP = (OHP - (UPower * 1.15));
		cout << "\nYou draw your sword and attack the other doing" << (UPower * 1.15)<< "DAMAGE";
	}
	//if it is not active
	else {
		OHP = (OHP - UPower);
		cout << "\nYou draw your sword and attack the other doing" << UPower<< "DAMAGE";
...
void battle(User &player, Other &other) {
	...
	while ((player.UHP > 0) && (other.OHP > 0)&& (lose == false)) {
		cout << "\nwhat would you like to do ? \n||1=fight||2 = defend(75% less damage)||3 = BRAIN DRIVE(15% more damage for you for three turns)";
		int choice;
		cin >> choice;
		//input validation
		...
		//Brain dive choice
			...
			if (DriveTurnsLeft > 0) {
				...
				DriveTurnsLeft = 3;
				cout << "You call forth the power of Brain Drive +15% attack for three turns\n";
				Oattack(player.UHP, other.OPower);
